...
void GeneticAlgorithm::evolve(std::function<double(std::shared_ptr<HeuristicNode>)> evaluateFunc) {
    // 1. Calcular Fitness de todos em paralelo
    #ifdef _OPENMP
    static bool first = true;
    if (first) {
        std::cout << "OpenMP: Usando " << omp_get_max_threads() << " threads." << std::endl;
        first = false;
    }
    #endif
...
double GeneticAlgorithm::getBestFitness() const {
    if (population.empty()) return -1.0e18;
