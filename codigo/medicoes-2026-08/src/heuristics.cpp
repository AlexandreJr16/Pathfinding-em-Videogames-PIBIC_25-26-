...
vector<pair<int, int>> generatePivots(int n) {
  ...
  pair<int, int> pivot;
  // 1. O primeiro pivô é sempre aleatório (para garantir variação por semente)
  int tries = 0;
  ...
  while (pivotList.size() < n) {
    ...
    if (nextPivot.first != -1 && maxDist > 0) {
      ...
      // Fallback: Se o mapa estiver muito fragmentado ou esgotarem os pontos
      // úteis, sorteia o restante aleatoriamente para evitar loop infinito
      ...
      if (tries_fallback >= height * width * 2) break; // Não consegue mais pivôs
    }
  }
...
int heuristicaFormula(pair<int, int> s, pair<int, int> goal) {
  ...
  // Fallback para a fórmula original se nenhuma AST estiver definida
