...
bool verificaPontosConectados(pair<int, int> start, pair<int, int> goal) {
  if (start.first < 0 || start.first >= height || start.second < 0 ||
      start.second >= width || goal.first < 0 || goal.first >= height ||
      goal.second < 0 || goal.second >= width)
    return false;
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (actualX >= 0 && actualX < height && actualY >= 0 && actualY < width &&
        ...
        if (actualX == goal.first && actualY == goal.second)
          return true;
...
void loadMap(const string &filename) {
  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++) {
      grid[i][j] = (line[j] == '.');
      if (grid[i][j])
        originalTraversableCells++;
    }
  }
}

// pega raio dinâmico
int getDynamicSize() { return max(2, min(height, width) / 20); }
