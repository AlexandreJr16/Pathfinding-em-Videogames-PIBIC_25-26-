...
#include "synthesis/SimulatedAnnealing.h"
#include <bits/stdc++.h>
...
void writeCSVHeader(ofstream &file, const string &filename,
                    const string &header) {
...
Mode currentMode = Mode::DELTA; // default
...
int main(int argc, char *argv[]) {
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "--mode" && i + 1 < argc) {
      string modeStr = argv[++i];
      if (modeStr == "absolute")
        currentMode = Mode::ABSOLUTE;
      else if (modeStr == "delta")
        currentMode = Mode::DELTA;
      else if (modeStr == "admissibility")
        currentMode = Mode::ADMISSIBILITY;
      else if (modeStr == "hybrid")
        currentMode = Mode::HYBRID;
    }
  }
  ...
  writeCSVHeader(csvBase, "resultados_base.csv",
                 "implementacao,modo,mapa,semente,id_problema,heuristica,n_"
                 "pivos,expansoes,tempo_"
                 "ms,caminho_tamanho");

  ofstream csvRobustez("resultados_robustez.csv", ios::app);
  writeCSVHeader(
      csvRobustez, "resultados_robustez.csv",
      "implementacao,modo,mapa,semente,id_problema,tipo_degradacao,n_pivos,"
      "porcentagem_bloqueio,"
      "exp_orig_manh,exp_deg_manh,exp_orig_form,exp_deg_form,exp_orig_mem,exp_"
      "deg_mem,"
      "path_orig_manh,path_deg_manh,path_orig_form,path_deg_form,path_orig_mem,"
      "path_deg_mem,"
      "tempo_orig_manh_ms,tempo_deg_manh_ms,tempo_orig_form_ms,tempo_deg_form_"
      "ms,tempo_orig_mem_ms,tempo_deg_mem_ms");

  ofstream csvRatio("resultados_ratio.csv", ios::app);
  writeCSVHeader(csvRatio, "resultados_ratio.csv",
                 "implementacao,modo,mapa,semente,id_problema,caminho_otimo,"
                 "caminho_formula,ratio");

  ofstream csvSintese("resultados_sintese.csv", ios::app);
  writeCSVHeader(
      csvSintese, "resultados_sintese.csv",
      "implementacao,modo,mapa,populacao_inicial,n_geracoes,tempo_sintese_ms,"
      "formula_"
      "pre_sa,formula_pos_sa,tempo_sa_ms,admissibility_rate,fitness_final");
  ...
  for (const auto &config : mapas) {
    ...
    // sintese da heuristica
    if (currentMode == Mode::HYBRID) {
      computarDistPivos(3); // k=3 pivots for hybrid mode
    }

    auto tSinteseInicio = chrono::high_resolution_clock::now();
    HeuristicGrammar grammar(currentMode, 42); // Semente fixa para síntese
    ...
    int trainingSize =
        min((int)scenario.size(), max(150, (int)scenario.size() / 4));
    ...
    vector<double> trainingRealCost(fitnessProblems.size());
    for (size_t i = 0; i < fitnessProblems.size(); ++i) {
      auto res = aStar(fitnessProblems[i].start, fitnessProblems[i].goal,
                       heuristicaManhattan);
      trainingManhattanExp[i] = max(1, res.expansions);
      if (currentMode == Mode::ADMISSIBILITY) {
        auto resZero = aStar(fitnessProblems[i].start, fitnessProblems[i].goal,
                             heuristicaZero);
        trainingRealCost[i] = max(0.0, (double)resZero.path.size() - 1.0);
      }
    }

    cout << "  > Evoluindo heurística..." << endl;
    auto lambda_fitness = [&](std::shared_ptr<HeuristicNode> formula) {
      long long totalExpManh = 0, totalExpForm = 0;
      int inadmissibleCount = 0;
      for (size_t i = 0; i < fitnessProblems.size(); ++i) {
        auto h_local = [&](pair<int, int> s, pair<int, int> g) {
          return (int)formula->evaluate({s.first, s.second},
                                        {g.first, g.second});
        };
        auto res =
            aStar(fitnessProblems[i].start, fitnessProblems[i].goal, h_local);
    ...
    int maxGenerations = (currentMode == Mode::ADMISSIBILITY) ? 20 : 100;
    for (int g = 0; g < maxGenerations; g++) {
      ga.evolve(lambda_fitness);
    }
    currentFormula = ga.getBest();
    auto tSinteseFim = chrono::high_resolution_clock::now();
    double tempoSinteseMs =
        chrono::duration<double, milli>(tSinteseFim - tSinteseInicio).count();

    cout << "  > Otimizando heurística (SA)..." << endl;
    auto tSAInicio = chrono::high_resolution_clock::now();
    SimulatedAnnealing sa(grammar, operators);
    auto optimizedFormula = sa.optimize(currentFormula, lambda_fitness, 100);
    double fitness_pre = lambda_fitness(ga.getBest());
    double fitness_pos = lambda_fitness(optimizedFormula);

    string formulaPre = currentFormula->toString();
    if (fitness_pos < fitness_pre) {
      optimizedFormula = currentFormula;
      fitness_pos = fitness_pre;
    }
    currentFormula = optimizedFormula;
    string formulaPos = currentFormula->toString();

    auto tSAFim = chrono::high_resolution_clock::now();
    double tempoSAMs =
        chrono::duration<double, milli>(tSAFim - tSAInicio).count();

    double admissibility_rate = 0.0;
    if (currentMode == Mode::ADMISSIBILITY) {
      int inadmissibleCount = 0;
      for (size_t i = 0; i < fitnessProblems.size(); ++i) {
        double h_val = currentFormula->evaluate(
            {fitnessProblems[i].start.first, fitnessProblems[i].start.second},
            {fitnessProblems[i].goal.first, fitnessProblems[i].goal.second});
        if (h_val > trainingRealCost[i])
          inadmissibleCount++;
      }
      admissibility_rate = (double)inadmissibleCount / fitnessProblems.size();
    }

    csvSintese << "ours," << modeToStr(currentMode) << "," << config.name
               << ",80," << maxGenerations << "," << tempoSinteseMs << ",\""
               << formulaPre << "\",\"" << formulaPos << "\"," << tempoSAMs
               << "," << admissibility_rate << "," << fitness_pos << endl;
    cout << "  > Heurística Campeã: " << formulaPos
         << " (Fitness: " << fitness_pos << ")" << endl;
    ...
    for (int semente : sementes) {
      ...
      for (size_t i = 0; i < scenario.size(); ++i) {
        ...
        auto resM =
            aStar(scenario[i].start, scenario[i].goal, heuristicaManhattan);
        auto t2 = chrono::high_resolution_clock::now();
        baseManh[i] = {resM.expansions,
                       chrono::duration<double, milli>(t2 - t1).count(),
                       (int)resM.path.size() - 1};
        csvBase << "ours," << modeToStr(currentMode) << "," << config.name
                << "," << semente << "," << i << ",manhattan,0,"
                << baseManh[i].exp << "," << baseManh[i].time << ","
                << baseManh[i].path << endl;

        // Formula
        t1 = chrono::high_resolution_clock::now();
        auto resF =
            aStar(scenario[i].start, scenario[i].goal, heuristicaFormula);
        t2 = chrono::high_resolution_clock::now();
        baseForm[i] = {resF.expansions,
                       chrono::duration<double, milli>(t2 - t1).count(),
                       (int)resF.path.size() - 1};
        csvBase << "ours," << modeToStr(currentMode) << "," << config.name
                << "," << semente << "," << i << ",formula,0,"
                << baseForm[i].exp << "," << baseForm[i].time << ","
                << baseForm[i].path << endl;
        ...
        csvRatio << "ours," << modeToStr(currentMode) << "," << config.name
                 << "," << semente << "," << i << ","
                 << pathOptimal << ","
                 << pathFormula << "," << ratio << endl;
      }
      ...
      for (int n : numPivos) {
        ...
        for (size_t i = 0; i < scenario.size(); ++i) {
          auto t1 = chrono::high_resolution_clock::now();
          auto res =
              aStar(scenario[i].start, scenario[i].goal, heuristicaMemoryBased);
          auto t2 = chrono::high_resolution_clock::now();
          currentMem[i] = {res.expansions,
                           chrono::duration<double, milli>(t2 - t1).count(),
                           (int)res.path.size() - 1};
          csvBase << "ours," << modeToStr(currentMode) << "," << config.name
                  << "," << semente << "," << i << ",memory," << n << ","
                  << currentMem[i].exp << "," << currentMem[i].time << ","
                  << currentMem[i].path << endl;
        }
      ...
      for (auto &deg : degs) {
        ...
        for (double pct : porcentagensPadrao) {
          ...
          for (int n : numPivos) {
            ...
            for (int i : problemasValidos) {
              auto t1 = chrono::high_resolution_clock::now();
              auto resMem = aStar(scenario[i].start, scenario[i].goal,
                                  heuristicaMemoryBased);
              auto t2 = chrono::high_resolution_clock::now();
              PerfData dMem = {resMem.expansions,
                               chrono::duration<double, milli>(t2 - t1).count(),
                               (int)resMem.path.size() - 1};

              csvRobustez << "ours," << modeToStr(currentMode) << ","
                          << config.name << "," << semente << "," << i << ","
                          << deg.name << "," << n << "," << pct << ","
        ...
        if (deg.name == "Radial")
          sobreviventes[config.name][semente] = problemasValidos.size();
      }
      grid = gridOriginal;
      csvBase.flush();
      csvRobustez.flush();
  ...
  double tempoTotalMs =
      chrono::duration<double, milli>(tExperimentoFim - tExperimentoInicio)
          .count();
  ...
  cout << "  > Total de Linhas no CSV de Robustez: " << totalLinhasRobustez
       << endl;
