...
public:
   ...
   * @param evaluateFunc Função que recebe uma fórmula e retorna o seu fitness
   * (speedup).
   */
  void
  evolve(std::function<double(std::shared_ptr<HeuristicNode>)> evaluateFunc);
...
private:
  ...
  double lambda = 0.0005; // Penalidade por tamanho baixíssima para permitir
                          // fórmulas complexas
