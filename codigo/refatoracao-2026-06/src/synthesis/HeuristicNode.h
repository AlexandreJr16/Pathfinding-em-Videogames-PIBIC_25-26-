...
public:
  ...
  /**
   * @brief Retorna um ponteiro para o n-ésimo nó da subárvore (ordem
   * pré-fixada).
   */
  virtual std::shared_ptr<HeuristicNode> getNode(size_t index,
                                                 size_t &currentIndex) = 0;

  /**
   * @brief Substitui o n-ésimo nó por um novo nó.
   */
  virtual bool replaceNode(size_t index, std::shared_ptr<HeuristicNode> newNode,
                           size_t &currentIndex) = 0;
...
extern std::vector<std::vector<std::vector<int>>> distPivos;

class TerminalNode : public HeuristicNode,
                     public std::enable_shared_from_this<TerminalNode> {
public:
  enum Type {
    DELTAX,
    DELTAY,
  ...
  TerminalNode(Type t, double val = 0.0, int pIdx = 0)
      : type(t), constantValue(val), pivotIndex(pIdx) {
    _cachedSize = 1;
    _cachedDepth = 1;
  }
  ...
  double evaluate(const Point &s, const Point &g) const override {
    switch (type) {
    case DELTAX:
      return std::abs(s.x - g.x);
    case DELTAY:
      return std::abs(s.y - g.y);
    case X1:
      return s.x;
    case Y1:
      return s.y;
    ...
    case Y2:
      return g.y;
    case PIVOT_DIST_S:
      if (pivotIndex >= 0 && pivotIndex < (int)distPivos.size() && s.x >= 0 &&
          s.x < (int)distPivos[pivotIndex].size() && s.y >= 0 &&
          s.y < (int)distPivos[pivotIndex][s.x].size()) {
        return distPivos[pivotIndex][s.x][s.y];
      }
      return 0.0;
    case PIVOT_DIST_G:
      if (pivotIndex >= 0 && pivotIndex < (int)distPivos.size() && g.x >= 0 &&
          g.x < (int)distPivos[pivotIndex].size() && g.y >= 0 &&
          g.y < (int)distPivos[pivotIndex][g.x].size()) {
        return distPivos[pivotIndex][g.x][g.y];
      }
      return 0.0;
    case CONSTANT:
  ...
  std::string toString() const override {
    ...
    case Y2:
      return "y2";
    case PIVOT_DIST_S:
      return "d(p" + std::to_string(pivotIndex) + ",s)";
    case PIVOT_DIST_G:
      return "d(p" + std::to_string(pivotIndex) + ",g)";
    case CONSTANT: {
      char buffer[32];
  ...
  std::shared_ptr<HeuristicNode> clone() const override {
    return std::make_shared<TerminalNode>(type, constantValue, pivotIndex);
  }

  std::shared_ptr<HeuristicNode> getNode(size_t index,
  ...
  bool replaceNode(size_t index, std::shared_ptr<HeuristicNode> newNode,
                   size_t &currentIndex) override {
...
class UnaryNode : public HeuristicNode,
                  public std::enable_shared_from_this<UnaryNode> {
public:
  enum Type { SQRT, ABS, NEG, SQR };

  UnaryNode(Type t, std::shared_ptr<HeuristicNode> child)
      : type(t), operand(child) {
...
class BinaryNode : public HeuristicNode,
                   public std::enable_shared_from_this<BinaryNode> {
public:
  enum Type { ADD, SUB, MUL, DIV, MAX, MIN };

  BinaryNode(Type t, std::shared_ptr<HeuristicNode> l,
             std::shared_ptr<HeuristicNode> r)
