...
def construir_figura(manh_agg, form_agg, mem_agg):
    ...
    # GridSpec único de 2 linhas × 6 colunas
    # Top: cols 0-1, 2-3, 4-5  (3 subplots × 2 colunas cada)
    # Bot: cols 1-2, 3-4        (2 subplots × 2 colunas cada, centrados)
    ...
    ax_top += [fig.add_subplot(gs[0, 2:4], sharey=ax_top[0]),
               fig.add_subplot(gs[0, 4:6], sharey=ax_top[0])]
    ax_bot  = [fig.add_subplot(gs[1, 1:3], sharey=ax_top[0]),
               fig.add_subplot(gs[1, 3:5], sharey=ax_top[0])]
    ax_list = ax_top + ax_bot
    ...
    ax_top[0].set_ylabel("Fator de degradação (expansões)", fontsize=11)
    ax_bot[0].set_ylabel("Fator de degradação (expansões)", fontsize=11)

    # Ocultar ytick labels nos subplots que não são o primeiro de cada linha
    for ax in ax_top[1:] + ax_bot[1:]:
